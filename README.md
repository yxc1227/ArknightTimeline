...
php artisan test                          # 运行测试（313 项 / 2780 断言）
...
| `/operators` | 人员简介列表。卡片网格 + 名称/代号/头衔/种族搜索 + 阵营筛选（含子阵营）；可在「干员 / 历史人物」之间切换（干员有代号，历史人物有头衔与在位期） |
...
- **测试**：PHPUnit 12（313 项 / 2780 断言，含种子数据完整性与守卫测试）
...
| ↳ 同书派生的字典 | 24 种族 · 46 地名 · 20 组织 · 20 词条 | 种族（第四章）、政区层级（第五章全部十九卷）、组织（第六章）、术语（全书）—— 供人物、条目与检索挂载，**四个大类各自成页**（`/places` `/organizations` `/races` `/terms`）；地名可记别名（书里并用过多种写法时，如「乌萨斯帝国 / 乌萨斯」），并有上下层级 | 独立于条目，按需补充 |
