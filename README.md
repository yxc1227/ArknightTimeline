...
php artisan test                          # 运行测试（320 项 / 2837 断言）
...
- **测试**：PHPUnit 12（320 项 / 2837 断言，含种子数据完整性与守卫测试）
...
| ↳ 泰拉人员 | 443 干员 · 18 历史人物 · 2 剧情人物 · 4 塔卫二 | 干员名单来自 `docs/prts-干员一览.json`（PRTS「干员一览」的导出，431 条），导入时**只取**结构化事实（名字 / 代号 / 种族 / 势力 / 出身地）—— 简介、星级、职业、数值一律不入库，且**只新增不覆盖**人工核过的人 | 快照式：要更新就重新导出那份文件 |
| ↳ 人物归属 | 580 条归属 · 111 人多归属 | `character_faction` 枢轴（一人可同时属「深海猎人」与「阿戈尔」），顺序照来源的层序（小队 → 团体 → 国别）。**没有 `characters.faction_id` 这一列** —— 同一条归属只该有一个出处 | 由名单导入生成，人工核过的那条排在前面 |
...
| ↳ 阵营类型 | 41 阵营 = 20 政体 + 1 地域 + 20 组织 | `factions.kind`：政体与地域在地名树里有节点，组织（企业 / 团体 / 武装 / 机构）才进「组织」页 —— 否则「维多利亚」会与「莱茵生命」并列 | 新增阵营必须归类，落在「未归类」会被测试拦下 |
| ↳ 组织的世界归属 | 泰拉 15 · 塔卫二 5 | **推导**而非存储：自身在本世界有条目/人物即属于本世界；内部部门这类零链接的随最近的上级。阵营表刻意没有 world 列（罗德岛协建了终末地工业，强行切分会把一个组织拆成两份） | 零链接且上级也定不了世界的组织会被测试拦下（否则它会在两页都消失） |
...
- **人员的简介是本仓库写的，名单是导出的**：`docs/prts-干员一览.json` 只用来补**结构化事实**
  （名字 / 代号 / 种族 / 势力），导入**只新增不覆盖**；`description` 永远由本仓库撰写，
  没有写的就是「待补」。PRTS 的干员页是**核对依据**，不是简介的来源 ——
