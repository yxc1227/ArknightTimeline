...
- **前端零构建是有意取舍**：部署简单、无 Node 依赖，代价是没有组件化与类型检查。
  动效同样零依赖：原生 CSS 动画 + `IntersectionObserver`（入场错峰、滚动揭示、
  导航进度线），只用 opacity / transform，并完整尊重系统「减弱动态效果」
- **人员的简介是本仓库写的，名单是导出的**：`docs/prts-干员一览.json`（泰拉）与
  ...
  - **头像随仓库分发，清单留在仓库外**：`public/assets/avatars/`（PRTS 431 张 +
    终末地 Wiki 33 张，目录按**所属世界**命名 `terra/`、`talos/`，不用来源名 ——
    换一次抓取源不必改目录）；抓取清单 `docs/manifest.json` 记着来源 URL 与
    抓取机的本地路径，与名单同一批不入仓库。seed 时由 `App\Support\CharacterAvatars`
    按「世界 + 人物名」精确关联到 `characters.avatar`，缺失时照常跑完，
    人物卡退回「首字方块」（绝不渲染一张碎图）。
