...
final class Icons
{
    ...
    public const ICONS = [
        ...
        /*
            ...
            四类各自成页，因为它们回答的是四个不同的问题（见 routes/web.php 的分组注释）：
            地名 = 在哪里、组织 = 谁在做、种族 = 哪些人、词条 = 这词什么意思。

            图标因此按「提问」画，而不是把四类画成同一个「词典」的样子 ——
            顶栏里四枚并排，若都长成一本书，读者只能靠读下面那行小字区分，图标就白画了。
            四者共同的视觉底座是既有的节点方块（同 timeline 的节点、status-* 的中心点）。

            造型上都不新增母题：地名只用「圆 + 直线」（圆属 §1 允许的几何体），
            组织复用单角斜切与实心节点，种族是节点的分叉，词条是实心条 + 描边线。
        */
        // 地名 —— 定位标记。圆头（几何体）+ 两条直线收于一点，中心留方形孔 = 「在此处」
        'places' => '<circle cx="12" cy="9.2" r="3.8"/><path d="M8.5 10.67L12 19l3.5-8.33"/><rect x="11.1" y="8.3" width="1.8" height="1.8" fill="currentColor" stroke="none"/>',
        // 组织 —— 旗帜。杆 + 燕尾旗面 + 旗上徽记（有成员、有归属，但旗内不划边界）
        'organizations' => '<path d="M6.5 3.5V20.5"/><path d="M6.5 5.2H19.6L16.4 9.2l3.2 4H6.5z"/><rect x="9.2" y="7.1" width="2.4" height="2.4" fill="currentColor" stroke="none"/>',
        // 种族 —— 谱系分叉。一个分类节点向下分出两个成员节点：一本字典，人物挂在它上面
        'races' => '<rect x="10.2" y="4.8" width="3.6" height="3.6"/><path d="M12 8.4v2.4M5.6 10.8h12.8M5.6 10.8v3M18.4 10.8v3"/><rect x="3.8" y="13.8" width="3.6" height="3.6"/><rect x="16.6" y="13.8" width="3.6" height="3.6"/>',
        // 词条 —— 条目结构。实心条件 + 两行释义（实心＝术语本体，描边＝本仓库的转写概括）
        'terms' => '<rect x="4.5" y="5.6" width="6.6" height="2.2" fill="currentColor" stroke="none"/><path d="M4.5 12h15"/><path d="M4.5 16.6h9.5"/>',
